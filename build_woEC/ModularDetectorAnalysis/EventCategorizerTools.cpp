                                          ...
                                          double totCutAnniMax_larger, double totCutDeexMin, double totCutDeexMax, JPetStatistics& stats, bool saveHistos)
{
  ...
  vector<vector<const JPetPhysRecoHit*>> annihilaions;
  ...
  for (uint i = 0; i < event.getHits().size(); i++)
  {
    ...
    annihilaions.push_back(annih_temp);
  }
...
bool EventCategorizerTools::checkFor2GammaLifetime(const JPetEvent& event, std::vector<int> bad_ID, JPetStatistics& stats, bool saveHistos, 
                                                  double maxThetaDiff, double maxTimeDiff, double maxDOP, double totCutAnniMin, double totCutAnniMax, double totCutDeexMin, 
                                                  ...
                                                  double scatterTimeMin, double scatterTimeMax, double scatterAngleMin, double scatterAngleMax)
{
  ...
  for (uint i = 0; i < event.getHits().size(); i++)
  {
    ...
    if (checkToT(promptHit, totCutDeexMin, totCutDeexMax))
    {
      if(!(std::find(bad_ID.begin(), bad_ID.end(), event.getHits().at(i)->getScin().getID()) != bad_ID.end()))  prompts.push_back(promptHit);
    }
  }
  if (prompts.size() == 0)
  {
  ...
  {
    auto firstHit = dynamic_cast<const JPetPhysRecoHit*>(event.getHits().at(i));
    if (checkToT(firstHit, totCutAnniMin, totCutAnniMax)) annih_start++;
  }

  if (saveHistos){
    stats.fillHistogram("2g_stats_multi_annihilations", annih_start);
    stats.fillHistogram("2g_stats_multi_prompt", prompt_start);
    stats.fillHistogram("2g_stats_multi", event.getHits().size());
  }
  ...
  for (uint i = 0; i < event.getHits().size(); i++)
  {
    ...
    if (saveHistos)
    {
      stats.fillHistogram("none_2g_tot_lifetime", firstHit->getToT());
    }
  ...
  bool thetaCut = false, tDiffCut = false, scatterCut = false, DOPCut = false;
  ...
  for (auto pair2g : annihilaions)
  {
    for (auto prompt : prompts)
    {
      ...
      int scinID_prompt = prompt->getScin().getID();

      auto tot1 = pair2g.first->getToT();
      auto tot2 = pair2g.second->getToT();
      auto tot_prompt = prompt->getToT();
      ...
      thetaCut = checkRelativeAngles(pair2g.first->getPos(), pair2g.second->getPos(), maxThetaDiff);
      tDiffCut = false; 
      scatterCut = false;
    ...
    if (saveHistos)
    {
      ...
      stats.fillHistogram("2g_tot_lifetime", tot1);
      stats.fillHistogram("2g_tot_lifetime", tot2);
      stats.fillHistogram("2g_tot_all", tot1);
      stats.fillHistogram("2g_tot_all", tot2);
      stats.fillHistogram("2g_tot_all", tot_prompt);
      stats.fillHistogram("2g_theta_lifetime", theta);
      stats.fillHistogram("2g_timeDiff_theta_lifetime", timeDiff, theta);
      stats.fillHistogram("2g_timeDiff_lifetime", timeDiff);
      stats.fillHistogram("2g_DOP_lifetime", DOP);
      stats.fillHistogram("2g_time_annih_12_lifetime", pair2g.second->getTime(), pair2g.first->getTime());
      stats.fillHistogram("2g_scatter_test_time_lifetime", timeDiff - dist_annih1_annih2/kLightVelocity_cm_ps);
      stats.fillHistogram("2g_ID_all_lifetime", scinID_annih1);
      stats.fillHistogram("2g_ID_all_lifetime", scinID_annih2);
      if(fabs(timeDiff)>tdiff_min&&fabs(timeDiff)<tdiff_max){
        stats.fillHistogram("2g_xy_peak_lifetime", firstVec.X(), firstVec.Y());
        stats.fillHistogram("2g_zx_peak_lifetime", firstVec.Z(), firstVec.X());
        stats.fillHistogram("2g_xy_peak_lifetime", secondVec.X(), secondVec.Y());
        stats.fillHistogram("2g_zx_peak_lifetime", secondVec.Z(), secondVec.X());
        stats.fillHistogram("2g_ID_peak_lifetime", scinID_annih1);
        stats.fillHistogram("2g_ID_peak_lifetime", scinID_annih2);
        stats.fillHistogram("2g_time_annih_12_peak_lifetime", pair2g.first->getTime(), pair2g.second->getTime());
        stats.fillHistogram("2g_xy_source_peak_lifetime", annhilationPoint.X(), annhilationPoint.Y());
        stats.fillHistogram("2g_zx_source_peak_lifetime", annhilationPoint.Z(), annhilationPoint.X());
      }
      else{
        stats.fillHistogram("2g_ID_remain_lifetime", scinID_annih1);
        stats.fillHistogram("2g_ID_remain_lifetime", scinID_annih2);
    ...
    if (!(checkForScatter(pair2g.first, pair2g.second, stats, false, testType, scatterTestValue, scatterTimeMin, scatterTimeMax, scatterAngleMin,
                          scatterAngleMax))){
        scatterCut = true;
        stats.fillHistogram("lifetime_scatter_2g_prompt", lifetime);
        stats.fillHistogram("lifetime_scatter_2g_prompt_zoom", lifetime);
        stats.fillHistogram("scatter_2g_tot_lifetime", tot1);
        stats.fillHistogram("scatter_2g_tot_lifetime", tot2);
        stats.fillHistogram("scatter_2g_theta_lifetime", theta);
        stats.fillHistogram("scatter_2g_timeDiff_lifetime", timeDiff);
        stats.fillHistogram("scatter_2g_scatter_test_time_lifetime", timeDiff - dist_annih1_annih2/kLightVelocity_cm_ps);
        if(fabs(timeDiff)>tdiff_min&&fabs(timeDiff)<tdiff_max){
          stats.fillHistogram("scatter_2g_xy_peak_lifetime", firstVec.X(), firstVec.Y());
          stats.fillHistogram("scatter_2g_zx_peak_lifetime", firstVec.Z(), firstVec.X());
          stats.fillHistogram("scatter_2g_xy_peak_lifetime", secondVec.X(), secondVec.Y());
          stats.fillHistogram("scatter_2g_zx_peak_lifetime", secondVec.Z(), secondVec.X());
          stats.fillHistogram("scatter_2g_xy_source_peak_lifetime", annhilationPoint.X(), annhilationPoint.Y());
          stats.fillHistogram("scatter_2g_zx_source_peak_lifetime", annhilationPoint.Z(), annhilationPoint.X());
        }
    }
    if (fabs(timeDiff) < maxTimeDiff)
    {
      tDiffCut = true;
        stats.fillHistogram("lifetime_tdiff_2g_prompt", lifetime);
        stats.fillHistogram("lifetime_tdiff_2g_prompt_zoom", lifetime);
        stats.fillHistogram("tdiff_2g_tot_lifetime", tot1);
        stats.fillHistogram("tdiff_2g_tot_lifetime", tot2);
        stats.fillHistogram("tdiff_2g_theta_lifetime", theta);
        stats.fillHistogram("tdiff_2g_timeDiff_lifetime", timeDiff);
        stats.fillHistogram("tdiff_2g_scatter_test_time_lifetime", timeDiff - dist_annih1_annih2/kLightVelocity_cm_ps);
    }
      // Time difference cut
    if(thetaCut)
    {  
        stats.fillHistogram("lifetime_theta_2g_prompt", lifetime);
        stats.fillHistogram("lifetime_theta_2g_prompt_zoom", lifetime);
        stats.fillHistogram("theta_2g_tot_lifetime", tot1);
        stats.fillHistogram("theta_2g_tot_lifetime", tot2);
        stats.fillHistogram("theta_2g_theta_lifetime", theta);
        stats.fillHistogram("theta_2g_timeDiff_theta_lifetime", timeDiff, theta);
        stats.fillHistogram("theta_2g_timeDiff_lifetime", timeDiff);
        stats.fillHistogram("theta_2g_ID_all_lifetime", scinID_annih1);
        stats.fillHistogram("theta_2g_ID_all_lifetime", scinID_annih2);
        stats.fillHistogram("theta_2g_time_annih_12_lifetime", pair2g.first->getTime(), pair2g.second->getTime());
        stats.fillHistogram("theta_2g_scatter_test_time_lifetime", timeDiff - dist_annih1_annih2/kLightVelocity_cm_ps);
        if(fabs(timeDiff)>tdiff_min&&fabs(timeDiff)<tdiff_max){
          stats.fillHistogram("theta_2g_xy_first_peak_lifetime", firstVec.X(), firstVec.Y());
          stats.fillHistogram("theta_2g_zx_first_peak_lifetime", firstVec.Z(), firstVec.X());
          stats.fillHistogram("theta_2g_xy_second_peak_lifetime", secondVec.X(), secondVec.Y());
          stats.fillHistogram("theta_2g_zx_second_peak_lifetime", secondVec.Z(), secondVec.X());
          stats.fillHistogram("theta_2g_xy_source_peak_lifetime", annhilationPoint.X(), annhilationPoint.Y());
          stats.fillHistogram("theta_2g_zx_source_peak_lifetime", annhilationPoint.Z(), annhilationPoint.X());
          stats.fillHistogram("theta_2g_ID_peak_lifetime", scinID_annih1);
          stats.fillHistogram("theta_2g_ID_peak_lifetime", scinID_annih2); 
        }
    ...
    if(scatterCut && tDiffCut && thetaCut && DOPCut)
    {
      ...
      stats.fillHistogram("ap_2g_tot_lifetime", tot1);
      stats.fillHistogram("ap_2g_tot_lifetime", tot2);
      stats.fillHistogram("ap_2g_theta_lifetime", theta);
      stats.fillHistogram("ap_2g_timeDiff_lifetime", timeDiff);
      stats.fillHistogram("ap_2g_scatter_test_time_lifetime", timeDiff - dist_annih1_annih2/kLightVelocity_cm_ps);
      ...
      stats.fillHistogram("ap_xy_zoom_lifetime", annhilationPoint.X(), annhilationPoint.Y());
      stats.fillHistogram("ap_zx_zoom_lifetime", annhilationPoint.Z(), annhilationPoint.X());
      stats.fillHistogram("ap_zy_zoom_lifetime", annhilationPoint.Z(), annhilationPoint.Y());
      stats.fillHistogram("ap_pos_zoom_lifetime", annhilationPoint.Z(), annhilationPoint.X(), annhilationPoint.Y());
      annih_ap+=1;
    }
    }
  ...
  if(isLifetimeEvent){
    stats.fillHistogram("ap_2g_stats_events", annih_ap);}

  return isLifetimeEvent;
}

bool EventCategorizerTools::checkFor2GammaLifetime_exactly_3hits(const JPetEvent& event, std::vector<int> bad_ID, JPetStatistics& stats, bool saveHistos, 
                                                  double maxThetaDiff, double maxTimeDiff, double totCutAnniMin, double totCutAnniMax, double totCutDeexMin, 
                                                  double totCutDeexMax, const TVector3& sourcePos, ScatterTestType testType, double scatterTestValue, 
                                                  double scatterTimeMin, double scatterTimeMax, double scatterAngleMin, double scatterAngleMax)
{
  if (event.getHits().size() != 3)
  ...
  if (saveHistos){
  stats.fillHistogram("2g_stats_multi_annihilations", annihilaions.size());
  stats.fillHistogram("2g_stats_multi_prompt", prompts.size());
  stats.fillHistogram("2g_time_hit_12_lifetime", event.getHits().at(0)->getTime(), event.getHits().at(1)->getTime());
  }

  bool isLifetimeEvent = false;
  double tdiff_min = 3500, tdiff_max = 5000;

  bool thetaCut = false, tDiffCut = false, scatterCut = false;
  ...
  for (auto pair2g : annihilaions)
  {
    ...
    if(scatterCut && tDiffCut && thetaCut)
    {
      isLifetimeEvent = true;
      stats.fillHistogram("lifetime_ap_2g_prompt", lifetime);
      stats.fillHistogram("lifetime_ap_2g_prompt_zoom", lifetime);
                                                   ...
                                                   double scatterTimeMin, double scatterTimeMax, double scatterAngleMin, double scatterAngleMax)
{
  ...
  for (auto pair2g : annihilaions)
  {
    for (auto prompt : prompts)
    {
      ...
      bool tDiffCut = false, scatterCut = false;

      int scinID1 = pair2g.first->getScin().getID();
      int scinID2 = pair2g.first->getScin().getID();
      int multi = pair2g.first->getSignalA().getPMSignals().size() + pair2g.first->getSignalB().getPMSignals().size();
      stats.fillHistogram("2g_masking_timeDiff_lifetime", timeDiff);
      stats.fillHistogram("2g_masking_timeDiff_ID_lifetime", scinID1, timeDiff);
      stats.fillHistogram("2g_masking_timeDiff_ID_lifetime", scinID2, timeDiff);
    // Scatter cut
    if (!(checkForScatter(pair2g.first, pair2g.second, stats, false, testType, scatterTestValue, scatterTimeMin, scatterTimeMax, scatterAngleMin,
                          scatterAngleMax)))  scatterCut = true;
        
    if (fabs(timeDiff) < maxTimeDiff)  tDiffCut = true;
    if(fabs(timeDiff)>tdiff_min&&fabs(timeDiff)<tdiff_max) stats.fillHistogram("2g_hit_multi_scin", scinID1, multi);

    if(scatterCut && tDiffCut && thetaCut){ 
      isLifetimeEvent = true;   
      stats.fillHistogram("lifetime_ap_2g_masking_prompt", lifetime);
      stats.fillHistogram("lifetime_ap_2g_masking_prompt_zoom", lifetime);
      }

    if(thetaCut) stats.fillHistogram("theta_2g_hit_multi_scin", scinID1, multi);
...
bool EventCategorizerTools::checkFor3GammaLifetime(const JPetEvent& event, double minRelAngleCut, double maxTimeDiff, JPetStatistics& stats, 
                            bool saveHistos, double totCutAnniMin, double totCutAnniMax, double totCutDeexMin, double totCutDeexMax, 
                            ...
                            double scatterAngleMax)
{
  ...
  for (auto pair3g : annihilaions)
  {
    for (auto prompt : prompts)
    {
      ...
      vector<double> timeDiffs = {pair3g[0]->getTime() - pair3g[1]->getTime(), pair3g[1]->getTime() - pair3g[2]->getTime(), pair3g[2]->getTime() - pair3g[0]->getTime()};
      double timeDiff = *max_element(timeDiffs.begin(), timeDiffs.end(), 
                                    [](double a, double b){ return std::fabs(a) < std::fabs(b);});
      //if(timeDiff < 0) cout<<"Time diffs: "<<timeDiffs[0]<<", "<<timeDiffs[1]<<", "<<timeDiffs[2]<<", max: "<<timeDiff<<endl;

      if(checkToT(pair3g[0], totCutAnniMin, totCutAnniMax) && checkToT(pair3g[1], totCutAnniMin, totCutAnniMax) && 
        checkToT(pair3g[2], totCutAnniMin, totCutAnniMax)) totCut=true;
      if(fabs(timeDiff) < maxTimeDiff) tDiffCut = true;
      if(transformedX > minRelAngleCut) thetaCut=true;
      if(totCut&&thetaCut&&tDiffCut) isLifetimeEvent = true;
...
bool EventCategorizerTools::checkFor3GammaLifetime(const JPetEvent& event, vector<int> bad_ID, double minRelAngleCut, double maxTimeDiff, double maxDOP, JPetStatistics& stats, 
                            ...
                            double scatterAngleMax)
{
  ...
  for (uint i = 0; i < event.getHits().size(); i++)
  {
    ...
    if (checkToT(firstHit, totCutAnniMin, totCutAnniMax)) annih_start++;
      for (uint j = i + 1; j < event.getHits().size(); j++)
      {
        auto secondHit = dynamic_cast<const JPetPhysRecoHit*>(event.getHits().at(j));
        double timeDiff = secondHit->getTime() - firstHit->getTime();
        double dist_annih = calculateDistance(firstHit, secondHit);
        if (checkToT(firstHit, totCutDeexMin, totCutDeexMax)) stats.fillHistogram("3g_scatter_test_time_prompt_lifetime", timeDiff - dist_annih/kLightVelocity_cm_ps);
        if (checkToT(firstHit, totCutAnniMin, totCutAnniMax)&&checkToT(secondHit, totCutAnniMin, totCutAnniMax)) stats.fillHistogram("3g_scatter_test_time_lifetime", timeDiff - dist_annih/kLightVelocity_cm_ps);
      }  
  }
  ...
  bool thetaCut = false, tDiffCut = false, totCut = false;
  int annih_ap = 0;
  ...
  for (auto pair3g : annihilaions)
  {
    for (auto prompt : prompts)
    {
      ...
      bool totCut=false, thetaCut = false, tDiffCut = false, DOPCut = false;
      ...
      if (saveHistos)
      {
        ...
        stats.fillHistogram("3g_tot_all", tot1);
        stats.fillHistogram("3g_tot_all", tot2);
        stats.fillHistogram("3g_tot_all", tot3);
        stats.fillHistogram("3g_tot_all", tot_prompt);
        stats.fillHistogram("3g_DOP_lifetime", DOP);
	      stats.fillHistogram("3g_rel_angles_lifetime", transformedX, transformedY);
        for(int tdiff_ind = 0; tdiff_ind < timeDiffs.size(); tdiff_ind++) stats.fillHistogram("3g_timeDiff_lifetime", timeDiffs[tdiff_ind]);
        stats.fillHistogram("lifetime_3g_prompt", lifetime);
        stats.fillHistogram("lifetime_3g_prompt_zoom", lifetime);

        if(DOP < maxDOP){
          DOPCut = true;
          stats.fillHistogram("lifetime_DOP_3g_prompt", lifetime);
          ...
          for(int tdiff_ind = 0; tdiff_ind < timeDiffs.size(); tdiff_ind++) stats.fillHistogram("DOP_3g_timeDiff_lifetime", timeDiffs[tdiff_ind]);
        }
        if(fabs(timeDiff_annih) < maxTimeDiff){
          tDiffCut = true;
          stats.fillHistogram("lifetime_tdiff_3g_prompt", lifetime);
          stats.fillHistogram("lifetime_tdiff_3g_prompt_zoom", lifetime);
          ...
          for(int tdiff_ind = 0; tdiff_ind < timeDiffs.size(); tdiff_ind++) stats.fillHistogram("tdiff_3g_timeDiff_lifetime", timeDiffs[tdiff_ind]);
        }
        if(transformedX > minRelAngleCut)
        {
          thetaCut=true;
          stats.fillHistogram("lifetime_theta_3g_prompt", lifetime);
          stats.fillHistogram("lifetime_theta_3g_prompt_zoom", lifetime);
          ...
          for(int tdiff_ind = 0; tdiff_ind < timeDiffs.size(); tdiff_ind++) stats.fillHistogram("theta_3g_timeDiff_lifetime", timeDiffs[tdiff_ind]);
        }
        if(thetaCut&&tDiffCut&&DOPCut){
          isLifetimeEvent = true;
          ...
          for(int tdiff_ind = 0; tdiff_ind < timeDiffs.size(); tdiff_ind++) stats.fillHistogram("ap_3g_timeDiff_lifetime", timeDiffs[tdiff_ind]);
          annih_ap++;
        }
      }
